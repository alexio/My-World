...
struct TokenizerT_ {
	...
	char * delims; //array for separators
	char * stream; //array for token stream
	int position;	//keeps track of the current index of the token stream. Originally starts at 0 and then gets set to the index of the char after the end of a token
};
